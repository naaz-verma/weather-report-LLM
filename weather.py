...
# #tavily output 
# news_results = fetch_climate_news()
# print(news_results)
...
# #formatted output
# formatted_news = format_news_articles(news_results)
...
response = tool.invoke({"query": "climate change impact on agriculture in 2024"})
formatted_news = format_news_articles(response)
print(formatted_news)

full_content = "\n\n".join([article["content"][:1000] for article in response])
response_LLM = model.invoke([
    HumanMessage(content=f"Summarize the following information on climate change and agriculture in 2024:\n\n{full_content}")
])
